...
//!   likely COSMIC's `cosmic-comp` - unverified): push-based, see
//!   [`wlr_toplevel`]. Tried first on any Wayland session.
//! - Wayland with KWin's `plasma-window-management` (KDE Plasma): push-based,
//!   see [`plasma_window`]. Not verified against a real KWin session - built
//!   from the protocol spec, not live-tested like the wlr backend.
//! - X11 (including a plain X11 session on any EWMH window manager): push-
//!   based via `_NET_ACTIVE_WINDOW`, see [`x11`].
//! - Wayland with neither protocol advertised (GNOME/Mutter, which
